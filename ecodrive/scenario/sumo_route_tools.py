...
SUPPORTED_CARLA_VERSIONS = ("0.9.13",)
...
DEFAULT_MAP = "Town04"
DEFAULT_VEHICLE_TYPE = "vehicle.tesla.model3"
...
def _ensure_autoware_spawn_point_passthrough(container_name):
    """Ensure start_carla.launch forwards the automated spawn_point argument unchanged."""
    docker_binary = shutil.which("docker")
    patch_script = r"""
from pathlib import Path
...
rospy.init_node("automated_autoware_battery_stop", anonymous=True, disable_signals=True)
...
def _normalize_color_for_form(color_value):
    """Normalize a color value for automated config fields."""
    if color_value is None:
        return "white"
...
def read_autoware_ego_vtype_config():
    ...
    try:
        battery_failure_threshold = float(
            params.get("automated.battery.failureThreshold", 0.0)
        )
    except ValueError:
        battery_failure_threshold = 0.0
...
def stop_carla_server(version=None, timeout=15.0):
    ...
    if psutil is None:
        raise RuntimeError("psutil is required to stop CARLA.")
