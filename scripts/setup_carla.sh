...
usage() {
    cat <<'EOF'
Usage:
  ./scripts/setup_carla.sh [<CARLA_DIR> [0.9.13]]
...
What it does:
  ...
  - builds/starts the `autoware_mini` Docker container if Docker Compose is available

Notes:
  - `custom_<Town>.sumocfg` files are generated on demand by the simulation code.
  - the script creates backups under `<CARLA_DIR>/.customcosim-backups/<timestamp>/`.
  - if `<CARLA_DIR>` is omitted, the script auto-detects `carla/CARLA_0.9.13`
    first, then the legacy root-level `CARLA_0.9.13` folder inside this repository.
...
detect_version() {
    ...
    die "Could not infer CARLA version from '$carla_dir'. Pass 0.9.13 explicitly."
}
...
bootstrap_carla_dir() {
    ...
    info "The simulation code will generate custom_Town*.sumocfg on demand."
}
...
if [[ ${#detected_dirs[@]} -eq 0 ]]; then
    die "No local CARLA 0.9.13 installation found under the repository root (expected `carla/CARLA_0.9.13` or the legacy root-level `CARLA_0.9.13`)."
fi
