...
class BridgeHelper(object):
    ...
    @staticmethod
    def _set_sumo_vtype_attribute(type_id, setter_name, value, cast=str):
    ...
    @staticmethod
    def _set_sumo_vtype_color(type_id, color):
        if not color:
            return
        ...
        if isinstance(color, str):
            try:
                parsed_color = [int(float(component.strip())) for component in color.split(',')]
            except ValueError:
                parsed_color = color
    ...
    def _create_sumo_vtype(carla_actor):
        ...
        if type_id in BridgeHelper._VTYPES:
            type_specs = BridgeHelper._VTYPES[type_id]
            attr_setters = {
                'vClass': ('setVehicleClass', str),
                'guiShape': ('setShapeClass', str),
            ...
            for key, value in type_specs.items():
                if key == 'params':
                    continue
                if key == 'color':
                    BridgeHelper._set_sumo_vtype_color(type_id, value)
                elif key in attr_setters:
                    setter_name, cast = attr_setters[key]
                    BridgeHelper._set_sumo_vtype_attribute(type_id, setter_name, value, cast)

            params = type_specs.get('params', {})
