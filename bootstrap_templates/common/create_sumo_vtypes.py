...
def _split_vtype_specs(user_specs):
    """Split a vType definition into XML attributes and nested params."""
    attrs = dict(user_specs or {})
    params = attrs.pop('params', {})

    if not isinstance(params, dict):
        logging.warning('Ignoring invalid params specification: %s', params)
        params = {}
...
def write_vtype_xml(filename, vtypes):
    ...
    for vtype in vtypes:
        ...
        params = vtype.get('params', {})
        vtype_element = ET.SubElement(root, 'vType', attrs)

        for key, value in params.items():
            if value is None or str(value).strip() == '':
                continue
            ET.SubElement(vtype_element, 'param', {'key': str(key), 'value': str(value)})
...
def generate_vtype(vehicle):
    ...
    attr_specs, param_specs = _split_vtype_specs(user_specs)
    specs.update(attr_specs)
    return {
        'attributes': specs,
        'params': param_specs
    }
